...
if (fs.existsSync(ENGINE_IN)) {
  ...
  const engine = (raw.articles || []).map((a) => ({
    ...
    // v1.1 engine photo. Prefer the self-hosted compressed WebP
    // (scripts/optimize-images.mjs); fall back to the remote original.
    imageUrl: a.image_local || a.image_url || null,
    imageAlt: a.image?.alt || safeParse(a.images_json, [])[0]?.alt || a.title || "",
  }));
...
export type EngineArticle = {
  ...
  imageSlots: number;    // image placements the engine suggested
  imageUrl: string | null; // engine-drawn photo (public, includes ?v= timestamp)
  imageAlt: string;        // alt text in Hebrew
};
...
export type EngineFaq = { q: string; a: string };
export type EngineArticle = { id: string; keyword: string; title: string; slug: string; metaTitle: string; metaDescription: string; excerpt: string; bodyHtml: string; faq: EngineFaq[]; schemaJson: string; wordCount: number; approved: boolean; publishedAt: string | null; imageSlots: number; imageUrl: string | null; imageAlt: string; };
export const engineArticles: EngineArticle[] = [];
`, "utf8");
